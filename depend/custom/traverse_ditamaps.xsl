...
<xsl:stylesheet xmlns:db="http://docbook.org/ns/docbook"
    ...
    <xsl:element name="topichead">
     <xsl:attribute name="navtitle">
       <xsl:choose>
         ...
         <xsl:when test="document(@href)/*[contains(@class, ' topic/topic ')]/title"><xsl:value-of select="document(@href)/*[contains(@class, ' topic/topic ')]/title"/></xsl:when>
  ...
  <xsl:template name="create-topicrefs">
    ...
    <xsl:for-each select="$document//headerfile-name[1]">      
      ...
      <xsl:message>headerfile-name: <xsl:value-of select="$headerfile-file-name"/> </xsl:message>
      <xsl:element name="topicref">
        <xsl:call-template name="filtering-attribute-management"/>
        <xsl:attribute name="href" select="concat($path-out, $headerfile-file-name)"></xsl:attribute>
      </xsl:element>     
    </xsl:for-each>  
    <xsl:for-each select="$document//table[descendant::struct]">      
      <xsl:variable name="firmware-file-name"  select="concat('/', normalize-space(translate(title, ' ', '_')), '_', $topicref-id)"/>  
      <xsl:message>Struct <xsl:value-of select="@id"/> <xsl:value-of select="normalize-space(translate(title, ' ', '_'))"/></xsl:message>
      <xsl:element name="topicref">
        <xsl:call-template name="filtering-attribute-management"/>
        <xsl:attribute name="href" select="concat($path-out, $firmware-file-name)"></xsl:attribute>
      </xsl:element>     
    </xsl:for-each>     
  </xsl:template>
  ...
  <xsl:template name="create-files">
    ...
    <xsl:variable name="path-out">
      ...
      <xsl:for-each select="$document//table[descendant::struct]">
        <xsl:variable name="firmware-file-name" select="concat('/', normalize-space(translate(title, ' ', '_')), '_')"/>
    ...
    <xsl:for-each select="$document//headerfile-name">
      ...
      <xsl:message>TITLE3: <xsl:value-of select="//headerfile-name[1]"/> </xsl:message>
      <xsl:result-document href="{concat($OUTPUT-DIR-VAR, $path-out, $master-file-name)}">
        <xsl:element name="topic">
          <xsl:call-template name="filtering-attribute-management"/>
          <xsl:attribute name="id" select="@id"/>
          <xsl:element name="title"><xsl:value-of select="//headerfile-name[1]"/></xsl:element>
        </xsl:element>
      </xsl:result-document>
    </xsl:for-each>  
  </xsl:template>
