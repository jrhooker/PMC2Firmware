...
xmlns:date="http://exslt.org/dates-and-times">
    ...
    <xsl:template name="filtering-attribute-management">
        <xsl:variable name="attribute-count" select="count(@*[name(.) = 
            ...
            'raid-support', 'fw_package', 'revision')])"></xsl:variable>
        <xsl:variable name="attribute-names" select="@*[name(.) = 
            ('revision', 'audience', 'product', 'sp-version', 'component', 'media', 
            'pmc_phy', 'pmc_switch', 'pmc_package', 'sas-ports', 
            'pcie-host-ports', 'pcie-drive-ports', 'ddr-width', 
            'raid-support', 'fw_package', 'revision')]/name()"></xsl:variable>
       ...
       <!-- <xsl:for-each select="@*[not(name(.) = (
            ...
            'pmc_phy', 'pmc_switch', 'pmc_package', 'sas-ports', 
            'pcie-host-ports', 'pcie-drive-ports', 'ddr-width', 
            'raid-support', 'fw_package', 'revision', 'bg_color', 'text_color'))]"   >
            <xsl:copy></xsl:copy> 
        </xsl:for-each>       -->
    </xsl:template>
    
    <xsl:template name="form-otherprops">   
        <xsl:param name="attribute-count"></xsl:param>
        <xsl:param name="attribute-names"></xsl:param>
        <xsl:param name="collected-value"> </xsl:param>
        <xsl:param name="internal-count" select="1"></xsl:param>
        <xsl:choose>
            <xsl:when test="number($attribute-count) >= number($internal-count)">
                <xsl:variable name="values" select="tokenize(normalize-space(@*[name(.) = $attribute-names[$internal-count]]), ' ')"/>
                <xsl:call-template name="form-value">
                    <xsl:with-param name="collected-value" select="$collected-value"/>
                    <xsl:with-param name="values" select="$values"/>
                    <xsl:with-param name="current-name"><xsl:value-of select="$attribute-names[$internal-count]"/></xsl:with-param>
                    <xsl:with-param name="attribute-count" select="$attribute-count"/>
    ...
    <xsl:template name="form-value">        
        ...
        <xsl:param name="current-name"></xsl:param>
        <xsl:param name="collected-value"> </xsl:param>
        ...
        <xsl:variable name="prefix">
            <xsl:choose>
                ...
                <xsl:when test="number($attribute-internal-count) > 1 and $count = 1">and ((</xsl:when>  
            ...
            <xsl:when test="count($values) >= $count">
                <xsl:variable name="value-swap">
                    <xsl:choose>
                        ...
                        <xsl:otherwise><xsl:value-of select="$values[$count]"/></xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:call-template name="form-value">
                    <xsl:with-param name="values" select="$values"/>
                    ...
                    <xsl:with-param name="collected-value">
                        <xsl:value-of select="concat($collected-value, $prefix, $current-name, '=', $value-swap, $suffix)"/>
                    </xsl:with-param>
                    <xsl:with-param name="attribute-count" select="number($attribute-count)"/>
                    <xsl:with-param name="attribute-internal-count" select="number($attribute-internal-count)"/>
                    <xsl:with-param name="attribute-names" select="$attribute-names"/>
                </xsl:call-template>                
            </xsl:when>
            <xsl:when test="count($values) &lt; $count">
                <xsl:call-template name="form-otherprops">                    
                    ...
                    <xsl:with-param name="attribute-count" select="number($attribute-count)"/>
                    <xsl:with-param name="internal-count" select="number($attribute-internal-count) + 1"/>
                    <xsl:with-param name="attribute-names" select="$attribute-names"/>
                </xsl:call-template>                
            </xsl:when>
